...
const getPoolAddressName = (address: string): string => {
  const knownAddresses: IKnownAddresses = poolIndex?.payout_addresses;

  if (!knownAddresses) {
    return 'Unknown';
  }

  const foundEntry = Object.entries(knownAddresses).find(([key, value]) => {
    return key === address;
  });

  return foundEntry?.[1]?.name || 'Unknown';
}

export default getPoolAddressName;
